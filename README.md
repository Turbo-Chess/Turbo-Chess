...
- Giacomo Fantini:
Implementazione della griglia della scacchiera base e il posizionamento dei pezzi standard.
Gestione degli eventi di click per la selezione del pezzo.
Logica dei movimenti dei pezzi (Alfiere, Torre, Regina, Cavallo, Re).
Implementazione del sistema di promozione (finestra di selezione del pezzo).
Extra: Implementazione degli effetti casuali.

- Brando Fabbri:
Implementazione visiva di Scacco Matto, Patta, e Scacco (es: colore del Re in pericolo o finestre di notifica).
Regole Avanzate (Arrocco, En Passant, Scacco Matto, Patta).
Implementazione dell'Handler dei Turni e del Timer.
Extra: Sviluppo della view di selezione dei Loadout (interfaccia per scambiare pezzi standard/custom).

- Luca Bevilacqua:
Sviluppo di tutte le scene di navigazione (Menu Principale, Impostazioni).
Salva/Carica partita.
Interprete delle stringhe delle mosse, implementazione della View del Log delle Mosse.
Sistema di punteggio. Implementazione del bottone "Surrender" (termina la partita senza che sia avvenuto uno Scacco Matto o una Patta).
Extra: Implementazione del Replay e dei suoi controlli, Implementazione dei pezzi custom.
